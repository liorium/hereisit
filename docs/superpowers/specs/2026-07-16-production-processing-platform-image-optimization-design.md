...
- Delete inputs after processing and results after confirmed download or a short hard timeout.
...
Browser
  ...
  └─ production compression
       → API Worker
           ...
           ├─ issue short-lived R2 upload capability
...
- verifies R2 object metadata after upload;
...
R2 stores temporary input and result bytes only. Browser uploads use short-lived presigned capabilities
and random object keys. Original filenames never become object keys. A finalized upload is accepted only
after the control plane verifies the expected object, size, and job ownership.
...
- anonymous session and future tenant IDs;
...
Durable Objects are reserved for future tenant-level credit and concurrency serialization. Workflows are
reserved for genuine multi-stage document jobs such as OCR followed by compression and signing.
...
4. The Worker reserves weighted units and issues a five-minute upload capability.
5. The browser uploads directly to R2 and reports real byte progress.
6. The client finalizes the upload.
7. The Worker verifies the object and enqueues the job.
8. The container claims the job idempotently, processes it, verifies the result, and records actual
   resource use.
9. The input object is deleted after successful processing or terminal failure.
10. The result becomes available through a short-lived download capability.
11. Where the browser can confirm the full response, it acknowledges the download and triggers deletion.
12. Otherwise the result expires after at most 30 minutes.
...
R2 lifecycle timing is not used to promise a 30-minute deletion guarantee because lifecycle execution
is not exact. Monitoring alerts on old input objects, old result objects, deletion failures, and a
non-zero orphan count.

Application logs and analytics may contain:

- anonymous tenant or session hash;
- job ID;
...
- **Control-plane integration tests:** presigned upload, finalization, queue delivery, idempotency, usage
  settlement, cancellation, download, and deletion.
...
- **Security tests:** malformed files, decompression bombs, command injection, object-key confusion,
  cross-job access, and credential or signed-URL log leakage.
...
Automatic rollback to the previous server engine or safe local executor occurs when a reviewed threshold
is breached for:
...
Engine and codec build IDs are immutable. Rollback changes routing rather than rebuilding an old binary.
