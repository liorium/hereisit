...
describe("real Workerd FixedLengthStream and R2 semantics", () => {
  it("rejects a retired PDF input before storing any artifact", async () => {
    const key = objectKey();
    ...
    ).rejects.toMatchObject({ code: "INVALID_ARTIFACT_REQUEST" });
    expect(await env.JOB_OBJECTS.head(key)).toBeNull();
  });
