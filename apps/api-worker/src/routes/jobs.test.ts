...
describe("retired PDF admission", () => {
  it("rejects PDF before reserving storage or quota, even for a maintainer", async () => {
    const runtime = makeRuntime({
          ...
          Array.from(
            new Uint8Array(
              await crypto.subtle.digest(
                "SHA-256",
                new TextEncoder().encode(pdfCreateBody().anonymousSessionId),
              ),
            ),
            (byte) => byte.toString(16).padStart(2, "0"),
          ).join(""),
