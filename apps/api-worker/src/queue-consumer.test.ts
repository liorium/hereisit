...
describe("Queue batch disposition", () => {
  it.each([
    "image.optimize@1",
    "pdf.optimize@1",
  ])("only dispatches active image contracts (%s)", async (contractId) => {
    const item = queueMessage({
      ...message,
      contractId,
      resourceClass: contractId === "image.optimize@1" ? "image-standard-v1" : "pdf-standard-v1",
    });
    const consume = vi.fn(async () => "completed" as const);
    await consumeProcessingQueue(
      batchFor(item),
      { IMAGE_JOBS_QUEUE_NAME: "hereisit-image-jobs-local", IMAGE_JOBS_DLQ_NAME: "dlq" } as never,
      {
        consumeImage: consume,
        recordQueueOperations: async () => undefined,
      },
    );
    expect(consume).toHaveBeenCalledTimes(contractId === "image.optimize@1" ? 1 : 0);
