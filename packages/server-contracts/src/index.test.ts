...
describe("retired contract rejection", () => {
  it("rejects retired jobs and engine requests while accepting image jobs", () => {
    expect(serverJobMessageSchema.safeParse(createMessage()).success).toBe(true);
    expect(
      serverJobMessageSchema.safeParse({ ...createMessage(), contractId: "pdf.optimize@1" })
        .success,
    ).toBe(false);
    expect(
      anyEngineCreateJobRequestSchema.safeParse({ tool: "pdf.optimize", protocol: 1 }).success,
    ).toBe(false);
    expect(
      serverEngineJobStatusSchema.safeParse({ tool: "pdf.optimize", status: succeededStatus() })
        .success,
    ).toBe(false);
  });
});
