import type { ImagePipelineSpecV2, ToolPreset } from "@hereisit/tool-contracts";

export * from "./processing";

const base: Pick<ImagePipelineSpecV2, "version" | "autoOrient" | "metadata" | "sizeGoal"> = {
  version: 2,
  autoOrient: true,
  metadata: "strip",
  sizeGoal: { mode: "allow-growth" },
};

const defaultImagePreset: ToolPreset = {
  id: "web-1920",
  name: "웹용 이미지",
  description: "긴 변을 최대 1920px로 줄여 웹에서 빠르게 열려요.",
  badge: "추천",
  spec: {
    ...base,
    resize: { kind: "inside", maxWidth: 1920, maxHeight: 1920 },
    output: { format: "webp", compression: { mode: "quality", quality: 84 } },
  },
};

export const imagePresets: readonly ToolPreset[] = [
  {
    id: "balanced",
    name: "용량만 줄이기",
    description: "원본 형식과 크기를 유지하며 용량만 줄여요.",
    badge: "절약",
    spec: {
      ...base,
      resize: { kind: "none" },
      output: { format: "source", compression: { mode: "quality", quality: 82 } },
      sizeGoal: {
        mode: "smaller-only",
        minSavingsPercent: 1,
        minQuality: 35,
        maxAttempts: 6,
      },
    },
  },
  defaultImagePreset,
  {
    id: "convert-webp",
    name: "형식만 바꾸기",
    description: "크기는 그대로 두고 WebP 형식으로 바꿔요.",
    badge: "변환",
    spec: {
      ...base,
      resize: { kind: "none" },
      output: { format: "webp", compression: { mode: "quality", quality: 84 } },
    },
  },
  {
    id: "product-square",
    name: "상품 정사각형",
    description: "가운데를 기준으로 1000×1000px 정사각형을 만들어요.",
    badge: "판매자",
    spec: {
      ...base,
      resize: { kind: "cover", width: 1000, height: 1000 },
      output: {
        format: "jpeg",
        compression: { mode: "quality", quality: 85 },
        matte: "#ffffff",
      },
    },
  },
  {
    id: "crop-square",
    name: "정사각형 자르기",
    description: "가운데를 기준으로 1:1 비율로 잘라요.",
    badge: "1:1",
    spec: {
      ...base,
      resize: { kind: "cover", width: 1200, height: 1200 },
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "crop-landscape",
    name: "가로 3:2 자르기",
    description: "사진에 잘 맞는 3:2 비율로 잘라요.",
    badge: "3:2",
    spec: {
      ...base,
      resize: { kind: "cover", width: 1200, height: 800 },
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "crop-portrait",
    name: "세로 4:5 자르기",
    description: "피드에 쓰기 좋은 4:5 비율로 잘라요.",
    badge: "4:5",
    spec: {
      ...base,
      resize: { kind: "cover", width: 960, height: 1200 },
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "rotate-90",
    name: "오른쪽으로 90°",
    description: "시계 방향으로 한 번 회전해요.",
    badge: "90°",
    spec: {
      ...base,
      resize: { kind: "none" },
      rotation: 90,
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "rotate-180",
    name: "180° 뒤집기",
    description: "이미지를 거꾸로 회전해요.",
    badge: "180°",
    spec: {
      ...base,
      resize: { kind: "none" },
      rotation: 180,
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "rotate-270",
    name: "왼쪽으로 90°",
    description: "반시계 방향으로 한 번 회전해요.",
    badge: "270°",
    spec: {
      ...base,
      resize: { kind: "none" },
      rotation: 270,
      output: { format: "source", compression: { mode: "quality", quality: 90 } },
    },
  },
  {
    id: "social-square",
    name: "SNS 정사각형",
    description: "피드에 쓰기 좋은 1080×1080px 이미지로 잘라요.",
    badge: "콘텐츠",
    spec: {
      ...base,
      resize: { kind: "cover", width: 1080, height: 1080 },
      output: {
        format: "jpeg",
        compression: { mode: "quality", quality: 86 },
        matte: "#ffffff",
      },
    },
  },
];

export function findImagePreset(id: string): ToolPreset {
  return imagePresets.find((preset) => preset.id === id) ?? defaultImagePreset;
}
