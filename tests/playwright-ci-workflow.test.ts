...
describe("Playwright CI workflow", () => {
  ...
  it("removes PDF execution without bypassing independent image release evidence", () => {
    expect(workflow).not.toMatch(
      /apps\/pdf-engine|scripts\/[a-z-]*pdf[a-z-]*\.mjs|tests\/e2e\/pdf-/,
    );
    expect(existsSync(".github/workflows/pdf-quality-benchmark.yml")).toBe(false);
