...
describe("processing Wrangler generator", () => {
  it("generates an image-only deployment and retires the legacy PDF namespace", () => {
    const input = Object.fromEntries(
      Object.entries(validInput()).filter(([key]) => !key.startsWith("pdf")),
    );
    const config = generateProcessingWrangler(input);
    expect(config.containers.map((entry) => entry.class_name)).toEqual(["ImageEngineContainer"]);
    expect(config.queues.consumers).toHaveLength(2);
    expect(Object.keys(config.vars).some((key) => key.startsWith("PDF_"))).toBe(false);
    expect(config.migrations.at(-1)).toEqual({
      tag: "retire-pdf-engine-v1",
      deleted_classes: ["PdfEngineContainer"],
    });
  });
  ...
  it("generates an environment-bound deployment config without secrets", () => {
    ...
    expect(config.queues.producers).toEqual([{ binding: "IMAGE_JOBS", queue: input.queueName }]);
