...
describe("CI release evidence", () => {
  it("binds reviews to the remaining image engine, not a retired engine", () => {
    const validate = (ciEvidenceModule as Record<string, unknown>)
      .validateHostedImageCandidateBinding as (reports: unknown, candidate: unknown) => unknown;
    const candidate = {
      engine: { oci: { configDigest: `sha256:${"1".repeat(64)}` } },
      releaseAssets: { security: { gates: { imageEngine: { sha256: "2".repeat(64) } } } },
    };
    const reports = {
      fullCorpusBenchmark: { engineImageDigest: candidate.engine.oci.configDigest },
      commercialReview: { licenseGateSha256: "2".repeat(64) },
    };
    expect(validate(reports, candidate)).toBe(reports);
    expect(() =>
      validate(
        { ...reports, fullCorpusBenchmark: { engineImageDigest: `sha256:${"3".repeat(64)}` } },
        candidate,
      ),
    ).toThrow(/image candidate/);
    expect(() =>
      validate({ ...reports, commercialReview: { licenseGateSha256: "4".repeat(64) } }, candidate),
    ).toThrow(/image candidate/);
  });
  it("accepts only exact hosted-main check receipts bound to the source SHA", () => {
    ...
    const value = {
      ...
      checkRunId: 42,
      document: {
        schema: "hereisit-device-matrix-review@2",
        version: 2,
        passed: true,
        gitSha: input.gitSha,
        sourceSha256: input.sourceSha256,
  ...
  it("binds protected reviewed reports to the exact current @3 candidate", async () => {
