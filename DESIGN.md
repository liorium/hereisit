...
hereisit:
  ...
  direction: "1996 catalog clarity with modern file-tool usability"
  principle: "Remove anything that does not help the user choose a tool, complete the task, understand the result, or download it. The retro language is a brand layer, never an interaction model."
  usability:
    - "A first-time visitor must understand the next action without reading instructions"
    - "Prefer one recommended path over presenting every option at once"
    - "Use plain Korean labels and reveal details only when the user asks for them"
    - "Never trade task clarity, speed, accessibility, or mobile comfort for decoration"
  retain:
    - "A literal black page frame and square, high-contrast surfaces"
    - "Chunky display type, numbered sections, and catalog-like information hierarchy"
    - "Flat ribbon color blocks for tool families"
    - "Native blue underlined links where a text action is clearer than another button"
  exclude:
    - "Times New Roman for Korean body copy"
    - "Decorative stickers, fake awards, product photography, and nostalgic clutter"
    - "Multiple competing CTA colors, pill badges, soft shadows, and rounded card grids"
    - "Retro browser behavior such as tiny targets, dense navigation, popups, or unclear progress"
  typography:
    display: "Arial Black, Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif"
    body: "Pretendard, Apple SD Gothic Neo, Noto Sans KR, system-ui, sans-serif"
    data: "ui-monospace, SFMono-Regular, Menlo, monospace"
    rules:
      - "Use display type only for brand and page titles; body copy stays Korean sans-serif"
      - "Use tabular numerals for bytes, percentages, page counts, and elapsed time"
      - "Keep explanatory lines under 65 Korean characters where possible"
  palette:
    frame: "#000000"
    canvas: "#fffef8"
    ink: "#000000"
    primary-action: "#e91d2a"
    ...
    error: "#b00020"
    image-ribbon: "#b3bd95"
    document-ribbon: "#9ab6c8"
    conversion-ribbon: "#e6915d"
    planned-ribbon: "#d8d8d0"
  layout:
    frame: "8px desktop, 4px mobile; canvas always remains inside it"
    header: "Black brand band plus a compact white navigation band; no floating glass panels"
    content-width: "1280px maximum with 24px desktop and 16px mobile gutters"
    home: "One dominant file-first launcher, then horizontal domain tabs and variable-width ribbon tool rows"
    catalog: "Search and filters form one compact control strip; results are a scan-friendly list/grid, not equal marketing cards"
    tool-page: "The workbench appears above the fold; title and privacy disclosure are concise and do not duplicate the catalog description"
    mobile: "Single column, horizontal-scroll tabs, 48px targets, and a sticky primary action only while it is useful"
  task-flow:
    setup:
      - "Show accepted formats and limits beside the picker"
      - "Expose one recommended preset; place optional controls in one details section"
      - "State local or server processing immediately beside the run action"
    processing:
      - "Replace the setup UI with one honest phase label, count, progress bar, and cancel action"
      - "Never show decorative loaders or fabricated time remaining"
    result:
      - "Show only before size, after size, reduction, one primary download, and start-over"
      - "For batches, show one ZIP download first and keep per-file results in progressive disclosure"
      - "If no smaller output exists, say that directly and offer the original download"
    failure:
      - "Keep the file selection when retry is safe and present one actionable recovery sentence"
  components:
    primary-button: "Red rectangle, black 2px border, minimum 48px height, white bold label, 2px pressed translation"
    secondary-button: "Canvas rectangle, black 2px border, minimum 44px height"
    tool-ribbon: "White title strip over one family tint; square edges; no shadow; entire row is clickable"
    input-zone: "White or family tint surface, black dashed 2px border, explicit file limits, no oversized illustration"
    result-panel: "Black title band and white body; measurements use tabular data type"
    status: "Color is supplementary; status always includes a text label"
  accessibility:
    - "Maintain visible 3px focus rings and keyboard-operable tabs"
    - "Honor prefers-reduced-motion and use only transform or opacity transitions"
    - "Keep 4.5:1 body-text contrast and never encode tool family or status by color alone"
    - "Keep the existing semantic headings, live regions, cancellation, and direct-download behavior"
